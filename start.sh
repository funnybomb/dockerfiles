...
if [ $(id -u) == 0 ] ; then
    ...
    # Only attempt to change the fcy username if it exists
    if id fcy &> /dev/null ; then
        echo "Set username to: $NB_USER"
        usermod -d /home/$NB_USER -l $NB_USER fcy
    fi
    ...
    if [[ "$NB_USER" != "fcy" ]]; then
        ...
        if [[ ! -e "/home/$NB_USER" ]]; then
            ...
            mv /home/fcy "/home/$NB_USER"
        fi
        # if workdir is in /home/fcy, cd to /home/$NB_USER
        if [[ "$PWD/" == "/home/fcy/"* ]]; then
    ...
    if [[ "$NB_UID" == "$(id -u fcy)" && "$NB_GID" == "$(id -g fcy)" ]]; then
        ...
        if [[ "$STATUS" != "0" ]]; then
            if [[ -w /etc/passwd ]]; then
                ...
                cat /etc/passwd | sed -e "s/^fcy:/nayvoj:/" > /tmp/passwd
                echo "fcy:x:$(id -u):$(id -g):,,,:/home/fcy:/bin/bash" >> /tmp/passwd
